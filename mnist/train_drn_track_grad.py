...
def train(args, model, optimizer, lr_scheduler, train_loader, val_loader, device):
    ...
    for batch_samples, batch_labels in tqdm(train_loader):
        ...
        if step % 10 == 0:
            with torch.no_grad():
                model_param_log = {f"{name}": torch.mean(param.grad).cpu().item() for name, param in model.named_parameters()}
                grad_log = {f"{name}_grad": torch.max(param.grad).cpu().item() for name, param in model.named_parameters()}
...
if __name__ =="__main__":
    ...
    mlflow.set_experiment(experiment_name="track-gradients")
    param_list = [
        {
            "hidden_q": 100,
            "num_bins": 11,
    ...
    with mlflow.start_run(nested=True):
        ...
        #     params_config = json.loads(file.read())
            ...
            for key, value in param_list[1].items():
